...
if suite >= 0
    @info " "
    @info "Testing if solid tides module is imported"
    if isdefined(Love, :compute_y)
        @info "Pass"
    else
        @warn "Fail"
        failed += 1
...
if suite >= 0
    @info " "
    @info "Testing if fluid tides module is imported"
...
if suite > 2
    ...
    # -------------
    # Test 1D solid method (andrade)
    # -------------
    @info " "
    @info "Testing 1D solid method (Andrade)"
    ...
    power_prf, power_blk, imag_k2 = Obliqua.calc_solid_tides(omega, ecc, rho, radius, visc, shear, bulk; ncalc=ncalc, material="andrade")
    test_pass = true
    ...
    # -------------
    # Test 1D solid method (maxwell)
    # -------------
    @info " "
    @info "Testing 1D solid method (Maxwell)"
    omega, ecc, rho, radius, visc, shear, bulk, ncalc = load.load_interior("$RES_DIR/interior_data/test_mantle.json", false)
...
if suite > 4
    ...
    # -------------
    # Test 1D solid method with mush interface (andrade)
    # -------------
    @info " "
    @info "Testing 1D solid method with mush interface (Andrade)"
    ...
    power_prf, power_blk, imag_k2 = Obliqua.calc_solid_tides_mush(omega, ecc, rho, radius, visc, shear, bulk, phi; ncalc=ncalc, material="andrade")
    test_pass = true
    ...
    # -------------
    # Test 1D solid method with mush interface (maxwell)
    # -------------
    @info " "
    @info "Testing 1D solid method with mush interface (Maxwell)"
    omega, ecc, rho, radius, visc, shear, bulk, phi, ncalc = load.load_interior_mush("$RES_DIR/interior_data/test_mantle_mush.json", false)
    ...
    power_prf, power_blk, imag_k2 = Obliqua.calc_solid_tides_mush(omega, ecc, rho, radius, visc, shear, bulk, phi; ncalc=ncalc, material="maxwell")
    test_pass = true
...
if suite > 6
    ...
    # -------------
    # Test fluid method (minimal)
    # -------------
    @info " "
    @info "Testing fluid method (0D data)"
...
if suite > 8
    ...
    # -------------
    # Test fluid method (PROTEUS-like)
    # -------------
    @info " "
    @info "Testing fluid method (1D data)"
...
if suite > 10
    ...
    # -------------
    # Test mixed interior data validity
    # -------------
    @info " "
    @info "Testing mixed interior data validity"
    ...
    omega, axial, ecc, sma, S_mass, rho, radius, visc, shear, bulk, phi, ncalc = load.load_interior_mush_full("$RES_DIR/interior_data/test_mantle_mush_full_test.json", false)
