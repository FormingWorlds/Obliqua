...
open(output_file, "w") do io
    println(io, "# Obliqua Code Coverage Report")
    println(io, "")
    ...
    println(io, "---")
    println(io, "")
    println(io, "_Generated by `test/runcoverage.jl`_")
end

@info "Coverage report written to: $output_file"
@info @sprintf("Overall coverage: %.1f%% (%d / %d lines)",
                overall_pct, total_covered, total_lines)
